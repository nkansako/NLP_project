...
def count_negations(book):
    ...
    freq = [f / (b / 5) * 100 for f in freq] # Change the list from total occurrences to percentage of total occurrences to match better with book lengths
...
def phonetic(book):
    ...
    for title, chapter in book.items():
        for line in chapter:
            ...
            first = first.replace("-","").replace("—", "").replace("Æ", "ae").replace("ü", "u").replace("ä", "a").replace("è", "e")
            last = last.replace("-","").replace("—", "").replace("Æ", "ae").replace("ü", "u").replace("ä", "a").replace("è", "e")
            try:
